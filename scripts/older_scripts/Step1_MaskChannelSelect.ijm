...
File.makeDirectory(dir + "CP_mask_images/"); 
dirSave=dir + "CP_mask_images/";
Dialog.create("Select channel for mask");
channelsList=newArray("C1-","C2-","C3-","C4-");
Dialog.addChoice("Mask Channel", channelsList);
Dialog.show()
...
for (j=0;j<files.length; j++){
	...
	run("Split Channels");
	selectWindow(maskChannel+name);
	run("Grays");
	saveAs(saveType, dirSave+saveName+"_MASK");
	run("Close All");
}