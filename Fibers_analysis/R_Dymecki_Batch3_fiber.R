...
#process data
df <- read.csv(here(path_to_csv_folder,filename), check.names=FALSE)
...
cols2summarize <- names(df)[grepl("*Skltn*", names(df))] #get columns with Area or Intensity in column name
...
measurements = c("Mean_BorderedCells_ObjectSkeleton_NumberBranchEnds_MrphlgclSkltn", 
                 "Mean_BorderedCells_ObjectSkeleton_NmbrNnTrnkBrnchs_MrphlgclSkltn",
                 "Mean_BorderedCells_ObjectSkeleton_NumberTrunks_MorphologiclSkltn",
                 "Mean_BorderedCells_ObjectSkeltn_TtlObjctSkltnLngth_MrphlgclSkltn")
desc = c("Number of Branch Termini",
         "Number of Trunk Branches",
         "Number of Trunks",
         "Total Skeleton Length")
index=4
ggplot(df_summary_mean, aes(x=Image_Metadata_Region, y=get(measurements[index]), fill=ExpGroup))+
...
#t test
grp1 <- df_summary_mean %>% filter(Image_Metadata_Region=="R4", ExpGroup=="11% IH") %>% pull(y_var)
grp2 <- df_summary_mean %>% filter(Image_Metadata_Region=="R4", ExpGroup=="RA") %>% pull(y_var)
t.test(grp1, grp2)

#overall
ggplot(total_df, aes(x=ExpGroup, y=get(measurements[index]), fill=ExpGroup))+
  scale_fill_manual(values=pal)+
  geom_boxplot(position = position_dodge(0.9))+
  geom_jitter(position = position_dodge(0.9), alpha=0.8, aes(color=ExpGroup))+
  scale_color_manual(values=pal_dark)+
...
###########
# Part 2: Filtering cells by their total projection length (to remove cells with small stubby projections)
#########
